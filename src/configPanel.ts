...
<body>
  ...
  <script nonce="${nonce}">
    ...
    function groupOptions(current) { const values = [...new Set(state.items.map((item) => item.group).filter((value) => value && value !== current))]; return [{ value: '', label: '无分组' }, ...values.map((value) => ({ value, label: value }))]; }
